...
def _parse_wsp_compensation(sample_el, transform_ns, data_type_ns):
    ...
    matrix_els = sample_el.findall(
        "%s:spilloverMatrix" % transform_ns, namespaces=sample_el.nsmap
    )
    ...
    params_els = matrix_el.find(
        "%s:parameters" % data_type_ns, namespaces=matrix_el.nsmap
    )
    param_els = params_els.findall(
        "%s:parameter" % data_type_ns, namespaces=matrix_el.nsmap
    )
    ...
    spill_els = matrix_el.findall(
        "%s:spillover" % transform_ns, namespaces=matrix_el.nsmap
    )
    ...
    for spill_el in spill_els:
        ...
        coefficient_els = spill_el.findall(
            "%s:coefficient" % transform_ns, namespaces=spill_el.nsmap
        )
        ...
        for co_el in coefficient_els:
            ...
            if value is None:
                raise ValueError(
                    "Matrix coefficient must have only 1 value (line %d)"
                    % co_el.sourceline
                )
...
def _parse_wsp_transforms(transforms_el, transform_ns, data_type_ns):
    ...
    for xform_el in xform_els:
        ...
        param_el = xform_el.find(
            "%s:parameter" % data_type_ns, namespaces=xform_el.nsmap
        )
        param_name = find_attribute_value(param_el, data_type_ns, "name")
        ...
        if xform_type == "linear":
            ...
            xforms_lut[param_name] = _transforms.LinearTransform(
                param_name, param_t=float(max_range), param_a=float(min_range)
            )
        elif xform_type == "log":
            ...
            xforms_lut[param_name] = _wsp_transforms.WSPLogTransform(
                param_name, offset=float(offset), decades=float(decades)
            )
            ...
            if param_length != "256":
                raise ValueError(
                    "FlowJo biex 'length' parameter value of %s is not supported."
                    % param_length
                )
            ...
            error_msg = (
                "FlowJo transform type '%s' is undocumented and not supported in FlowKit. "
                % xform_type
            )
            error_msg += (
                "Please edit the workspace in FlowJo and save all channel transformations as either "
...
def _convert_wsp_gate(wsp_gate, comp_matrix, xform_lut, ignore_transforms=False):
    ...
    for dim in wsp_gate.dimensions:
        ...
        new_dim = Dimension(
            dim_id, comp_ref, xform_id, range_min=new_dim_min, range_max=new_dim_max
        )
    ...
    if isinstance(wsp_gate, GMLPolygonGate):
        ...
        gate = PolygonGate(
            ...
            new_dims,
            vertices,
            use_complement=wsp_gate.use_complement,
        )
        ...
        raise NotImplementedError(
            "Ellipsoid gates are currently not supported. Please convert your ellipsoid gate to a polygon gate in FlowJo directly."
        )
        ...
        raise NotImplemented(
            "%s gates for FlowJo workspaces are not currently supported."
            % type(wsp_gate).__name__
        )
...
def _recurse_wsp_sub_populations(sub_pop_el, gate_path, gating_ns, data_type_ns):
    ...
    for pop_el in pop_els:
        ...
        if gate_type in ["RectangleGate", "PolygonGate", "EllipsoidGate"]:
            ...
            g = gate_class(
                gate_child_el, gating_ns, data_type_ns, use_complement=use_complement
            )
        else:
            g = gate_class(gate_child_el, gating_ns, data_type_ns)
        ...
        for el in sub_pop_els:
            gates.update(
                _recurse_wsp_sub_populations(
                    el, child_gate_path, gating_ns, data_type_ns
                )
            )
...
def _parse_wsp_groups(group_node_els, ns_map, gating_ns, data_type_ns):
    ...
    for group_node_el in group_node_els:
        ...
        wsp_groups[group_name] = {"gates": group_gates, "samples": group_samples}
...
def _parse_wsp_samples(sample_els, ns_map, gating_ns, transform_ns, data_type_ns):
    ...
    for sample_el in sample_els:
        ...
        sample_xform_lut = _parse_wsp_transforms(
            transforms_el, transform_ns, data_type_ns
        )
...
def parse_wsp(workspace_file_or_path, ignore_transforms=False, ignore_empty_gates=True):
    ...
    :return: dict
    """
    doc_type, root_xml, gating_ns, data_type_ns, transform_ns = _get_xml_type(
    ...
    wsp_samples = _parse_wsp_samples(
    ...
    for sample_id, sample_dict in wsp_samples.items():
        ...
        for sample_gate_id, sample_gate_dict in sample_dict["sample_gates"].items():
            ...
            tmp_gate = _convert_wsp_gate(
                sample_gate, sample_dict["comp"], sample_dict["transforms"], ignore_transforms=ignore_transforms
            )
            sample_gating_strategy.add_gate(
                tmp_gate, sample_gate_path, sample_id=sample_id
            )
    ...
    wsp_dict = {"groups": wsp_groups, "samples": processed_samples}
...
def extract_wsp_sample_data(workspace_file_or_path):
    ...
    raw_wsp_samples = _parse_wsp_samples(
        sample_els, ns_map, gating_ns, transform_ns, data_type_ns
    )
...
def _add_matrix_to_wsp(parent_el, prefix, matrix, ns_map):
    matrix_el = etree.SubElement(
        parent_el, "{%s}spilloverMatrix" % ns_map["transforms"]
    )
    ...
    for i_row, param_label in enumerate(matrix.detectors):
        spillover_el = etree.SubElement(
            matrix_el, "{%s}spillover" % ns_map["transforms"]
        )
        ...
        for i_col, param_label2 in enumerate(matrix.detectors):
            ...
            coef_el = etree.SubElement(
                spillover_el, "{%s}coefficient" % ns_map["transforms"]
            )
...
def _add_transform_to_wsp(parent_el, parameter_label, transform, ns_map):
    if isinstance(transform, _transforms.LinearTransform):
        ...
        raise NotImplementedError(
            "Transform type %s is not yet supported" % type(transform)
        )
...
def _add_polygon_gate(
    parent_el, gate, fj_gate_id, fj_parent_gate_id, gating_strategy, comp_prefix, ns_map
):
    ...
    if fj_parent_gate_id is not None:
        gate_instance_el.set(
            "{%s}parent_id" % ns_map["gating"], "ID%s" % fj_parent_gate_id
        )
...
def _add_rectangle_gate(
    ...
    gate_instance_el = etree.SubElement(
        parent_el, "{%s}RectangleGate" % ns_map["gating"]
    )
...
def _recurse_add_sub_populations(
    ...
    if isinstance(gate, PolygonGate):
        _add_polygon_gate(
            gate_el, gate, fj_id, parent_fj_id, gating_strategy, comp_prefix, ns_map
        )
        ...
        raise NotImplementedError(
            "Exporting %s gates is not yet implemented" % str(gate.__class__)
        )
...
def _add_sample_node_to_wsp(
    parent_el, sample_name, sample_id, gating_strategy, comp_prefix_lut, ns_map
):
...
def export_flowjo_wsp(gating_strategy, group_name, samples, file_handle):
    ...
    ns_map = {"gating": ns_g, "data-type": ns_dt, "transforms": ns_xform}
    ...
    for sample in samples:
        ...
        for dim, xform in dim_xform_lut.items():
            if xform is None:
                continue
            _add_transform_to_wsp(
                xforms_el, dim, gating_strategy.get_transform(xform), ns_map
            )
            ...
            for comp_prefix in comp_prefix_lut.values():
                _add_transform_to_wsp(
                    xforms_el,
                    comp_prefix + dim,
                    gating_strategy.get_transform(xform),
                    ns_map,
                )
        ...
        _add_sample_node_to_wsp(
            sample_el, sample.id, sample_id, gating_strategy, comp_prefix_lut, ns_map
        )
